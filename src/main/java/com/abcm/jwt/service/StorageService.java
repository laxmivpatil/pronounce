...
public class StorageService {
	  ...
	  @Value("${azure.storage.accountName}")
		...
		/*
		 * public String uploadFileOnAzure(MultipartFile file) throws IOException{ //
		 * Construct Azure Blob Storage URL
		 * 
		 * String blobServiceUrl = "https://satyaprofilestorage.blob.core.windows.net";
		 * String blobContainerUrl = String.format("%s/%s", blobServiceUrl,
		 * containerName);
		 * 
		 * // Get the original file extension String originalFileName =
		 * file.getOriginalFilename(); String fileExtension =
		 * originalFileName.substring(originalFileName.lastIndexOf('.'));
		 * 
		 * // Generate a unique name using timestamp and UUID String uniqueBlobName =
		 * Instant.now().toEpochMilli() + "_" + UUID.randomUUID().toString() +
		 * fileExtension;
		 * 
		 * // Create BlobClient with connection string BlobClientBuilder
		 * blobClientBuilder = new
		 * BlobClientBuilder().connectionString(container_string)
		 * .containerName(containerName).blobName(uniqueBlobName);
		 * 
		 * // Upload the file to Azure Blob Storage with the unique blob name try
		 ...
		 * Create a SAS token without expiration time OffsetDateTime expiryTime =
		 * OffsetDateTime.of(2099, 1, 1, 0, 0, 0, 0, ZoneOffset.UTC);
		 * 
		 * 
		 * // Assign read permissions to the SAS token BlobSasPermission sasPermission =
		 * new BlobSasPermission().setReadPermission(true);
		 * 
		 * // Set the start time for the SAS token (optional) OffsetDateTime startTime =
		 ...
		 * 
		 * // Generate SAS token for the blob String sasToken =
		 * blobClientBuilder.buildClient().generateSas(sasSignatureValues);
		 * 
		 * // Return the URL of the uploaded file with the SAS token String
		 * fileUrlWithSas = String.format("%s/%s?%s", blobContainerUrl, uniqueBlobName,
		 * sasToken); return fileUrlWithSas;
		 * 
		 * }
		 */
